...
def best_regression_model_storage_path(dataset: Dataset) -> Path:
    return (
        Path("results") / "models" / "regression" / dataset.tag() / "best_model.pickle"
    )
...
class VectorClassificationModelFromVectorRegressionModel(
    ...
    def __init__(
        ...
        super().__init__(
            [value_lt, value_gte], predicted_variable_name=COL_GEN_POPULARITY_CLASS
        )
    ...
    def _predict_class_probabilities(self, x: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            rows, index=x.index, columns=[self.value_gte, self.value_lt]
        )
...
class ClassificationModelFactory:
    ...
    DEFAULT_FEATURES = (
        FeatureName.MUSICAL_DEGREES,
        FeatureName.MUSICAL_CATEGORIES,
        FeatureName.TEMPO,
    ...
    def create_knn_orig(cls):
        return (
            SkLearnKNeighborsVectorClassificationModel(n_neighbors=1)
            .with_feature_generator(
                FeatureGeneratorTakeColumns(cls.COLS_USED_BY_ORIGINAL_MODELS)
            )
            .with_feature_transformers(DFTSkLearnTransformer(StandardScaler()))
    ...
    def create_random_forest_orig(cls):
        return (
            SkLearnRandomForestVectorClassificationModel(n_estimators=100)
            .with_feature_generator(
                FeatureGeneratorTakeColumns(cls.COLS_USED_BY_ORIGINAL_MODELS)
            )
            .with_feature_transformers(DFTSkLearnTransformer(StandardScaler()))
    ...
    def create_decision_tree_orig(cls):
        return (
            SkLearnDecisionTreeVectorClassificationModel(max_depth=2)
            .with_feature_generator(
                FeatureGeneratorTakeColumns(cls.COLS_USED_BY_ORIGINAL_MODELS)
            )
            .with_feature_transformers(DFTSkLearnTransformer(StandardScaler()))
    ...
    def create_xgb(
        ...
        **kwargs,
        ...
        return (
            XGBGradientBoostedVectorClassificationModel(
                min_child_weight=min_child_weight, **kwargs
            )
            .with_feature_collector(fc)
            .with_feature_transformers(fc.create_feature_transformer_one_hot_encoder())
    ...
    def create_classifier_from_best_regressor(cls, dataset):
        ...
        return VectorClassificationModelFromVectorRegressionModel(
            regression_model,
            threshold=dataset.threshold_popular,
            value_lt=dataset.class_negative,
            value_gte=dataset.class_positive,
        )
...
class RegressionModelFactory:
    ...
    def create_xgb(
        cls,
        name_suffix="",
        features: Sequence[FeatureName] = BASE_FEATURES,
        add_features: Sequence[FeatureName] = (),
        min_child_weight: Optional[float] = None,
        **kwargs,
    ):
    ...
    def create_xgb_meanpop_opt(cls):
        ...
        return cls.create_xgb(
            "-meanPop-opt", add_features=[FeatureName.MEAN_ARTIST_POPULARITY], **params
        )
