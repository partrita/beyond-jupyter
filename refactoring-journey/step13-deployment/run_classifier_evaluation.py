...
def main():
    ...
    experiment_name = (
        TagBuilder("popularity-classification", dataset.tag())
    ...
    evaluator_params = ClassificationEvaluatorParams(
        fractional_split_test_fraction=0.3, binary_positive_label=dataset.class_positive
    )
    cross_validator_params = VectorModelCrossValidatorParams(folds=3)
    ev = ClassificationModelEvaluation(
        io_data,
