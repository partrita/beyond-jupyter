...
def run_hyperopt(dataset: Dataset, model: Literal["xgb"] = "xgb", hours=2):
    ...
    if model == "xgb":
        ...
        def create_model(search_space_element: Dict[str, Any]):
            return RegressionModelFactory.create_xgb(
                add_features=[FeatureName.MEAN_ARTIST_POPULARITY],
                verbosity=0,
    ...
    evaluator_params = RegressionEvaluatorParams(
        fractional_split_test_fraction=0.3, fractional_split_random_seed=21
    )
    ...
    def objective(search_space_element: Dict[str, Any]):
        ...
        loss = (
            ev.perform_simple_evaluation(model)
            .get_eval_stats()
            .compute_metric_value(metric)
        )
    ...
    hyperopt.fmin(
        ...
        points_to_evaluate=initial_space,
    )
